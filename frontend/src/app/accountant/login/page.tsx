...
function AccountantLoginForm() {
  const router = useRouter();
  const search = useSearchParams();
  const next = search.get("next") || "/accountant";
  ...
  async function submit(e: FormEvent) {
    ...
    setLoading(true);
    try {
      const { data } = await api.post("/api/v1/accountant/auth/login", {
        email,
        password,
      });
      if (data.user.role !== "accountant") {
        throw new Error("This account is not an accountant.");
      }
  ...
  return (
    <AuthShell
      title="Welcome back"
      subtitle="Sign in to manage fees and payments."
      topRight={
        <>
          Accountant portal{" "}
    ...
    >
      <form onSubmit={submit} className="space-y-4">
        ...
        <div className="text-right text-[12px]">
          <Link
            href="/account/forgot-password?role=accountant"
