...
def login(req: LoginRequest, db: Annotated[Session, Depends(get_db)]):
    user = find_login_user(db, req.email, UserRole.teacher, req.password)
...
def refresh(req: RefreshRequest, db: Annotated[Session, Depends(get_db)]):
    ...
    if not user or not user.is_active or user.role != UserRole.teacher:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
...
def me(current_user: TeacherUser):
    return current_user
