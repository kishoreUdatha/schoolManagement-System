...
from app.core.deps import CoverManager, allow, allow_job
...
# one's own cover duties: teaching staff, or the cover job's holder (none, for office staff)
CoverReader = Annotated[User, Depends(allow_job(
    UserRole.school_admin, UserRole.principal, UserRole.teacher, permission="cover.manage"))]
# the leave list: teaching staff, or whoever holds the leave-deciding job
LeaveReader = Annotated[User, Depends(allow_job(
    UserRole.school_admin, UserRole.principal, UserRole.teacher, permission="studentleave.decide"))]
...
def cover_day(current_user: CoverManager, db: Db, on: date = Query(..., alias="date"),
...
def candidates(current_user: CoverManager, db: Db, entry_id: int, on: date = Query(..., alias="date")):
...
def assign(payload: AssignIn, current_user: CoverManager, db: Db):
...
def auto_assign(payload: AutoIn, current_user: CoverManager, db: Db):
...
def remove(sub_id: int, current_user: CoverManager, db: Db):
...
def stats(current_user: CoverManager, db: Db, start: Optional[date] = None, end: Optional[date] = None):
...
def list_blocks(current_user: CoverManager, db: Db):
...
def add_block(payload: UnavailabilityIn, current_user: CoverManager, db: Db):
...
def remove_block(block_id: int, current_user: CoverManager, db: Db):
...
def mine(current_user: CoverReader, db: Db, start: Optional[date] = None, end: Optional[date] = None):
...
def student_leaves(current_user: LeaveReader, db: Db, status_: Optional[StudentLeaveStatus] = Query(None, alias="status"),
