...
from app.core.deps import CurrentUser, HrManager, SchoolAdminUser
...
def list_openings(current_user: HrManager, db: Db, status_: Optional[OpeningStatus] = Query(None, alias="status")):
...
def create_opening(payload: OpeningIn, current_user: HrManager, db: Db):
...
def get_opening(opening_id: int, current_user: HrManager, db: Db):
...
def update_opening(opening_id: int, payload: OpeningIn, current_user: HrManager, db: Db):
...
def set_status(opening_id: int, current_user: HrManager, db: Db, value: OpeningStatus = Query(...)):
...
def delete_opening(opening_id: int, current_user: HrManager, db: Db):
    svc.delete_opening(db, current_user, opening_id)
    return Response(status_code=status.HTTP_204_NO_CONTENT)


@router.get("/pipeline", response_model=Pipeline)
def pipeline(current_user: HrManager, db: Db):
...
def list_candidates(current_user: HrManager, db: Db, search: Optional[str] = None):
...
def create_candidate(payload: CandidateIn, current_user: HrManager, db: Db):
...
def upload_resume(candidate_id: int, current_user: HrManager, db: Db, file: UploadFile = File(...)):
    return svc.candidate_to_read(db, svc.save_resume(db, current_user, candidate_id, file))


@router.get("/candidates/{candidate_id}/resume")
def download_resume(candidate_id: int, current_user: HrManager, db: Db):
...
def list_applications(current_user: HrManager, db: Db, opening_id: Optional[int] = None,
...
def create_application(payload: ApplicationIn, current_user: HrManager, db: Db):
...
def get_application(application_id: int, current_user: HrManager, db: Db):
...
def move_stage(application_id: int, payload: StageIn, current_user: HrManager, db: Db):
...
def schedule_interview(application_id: int, payload: InterviewIn, current_user: HrManager, db: Db):
...
def feedback(interview_id: int, payload: FeedbackIn, current_user: HrManager, db: Db):
...
def cancel_interview(interview_id: int, current_user: HrManager, db: Db):
...
def create_offer(application_id: int, payload: OfferIn, current_user: HrManager, db: Db):
...
def send_offer(offer_id: int, current_user: HrManager, db: Db):
...
def withdraw(offer_id: int, current_user: HrManager, db: Db, note: Optional[str] = None):
...
def hire(offer_id: int, payload: HireIn, current_user: HrManager, db: Db):
...
def list_types(current_user: HrManager, db: Db):
...
def create_type(payload: LeaveTypeIn, current_user: HrManager, db: Db):
    return svc.create_leave_type(db, current_user, payload)


@router.put("/leave-types/{type_id}", response_model=LeaveTypeRead)
def update_type(type_id: int, payload: LeaveTypeIn, current_user: HrManager, db: Db):
...
def delete_type(type_id: int, current_user: HrManager, db: Db):
    svc.delete_leave_type(db, current_user, type_id)
    return Response(status_code=status.HTTP_204_NO_CONTENT)


@router.get("/leave-balances", response_model=list[BalanceRead])
def balances(current_user: HrManager, db: Db, year: Optional[int] = None, user_id: Optional[int] = None):
...
def allot(payload: AllotIn, current_user: HrManager, db: Db):
...
def adjust(balance_id: int, payload: BalanceAdjustIn, current_user: HrManager, db: Db):
