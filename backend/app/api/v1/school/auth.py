...
def _check_tenant_active(db: Session, tenant_id: int) -> None:
    ...
    if not tenant or tenant.status != TenantStatus.active or not tenant.is_active:
        raise HTTPException(
            ...
            detail="Your school account is not active. Please contact support.",
        )


@router.post("/login", response_model=TokenResponse)
def login(req: LoginRequest, db: Annotated[Session, Depends(get_db)]):
    user = find_login_user(db, req.email, UserRole.school_admin, req.password)
...
def refresh(req: RefreshRequest, db: Annotated[Session, Depends(get_db)]):
    ...
    if not user or not user.is_active or user.role != UserRole.school_admin:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
...
def me(current_user: SchoolAdminUser):
    return current_user
