...
def alerts(current_user: SchoolAdminUser, db: Db, section_id: Optional[int] = Query(None)):
...
def immunizations_due(current_user: SchoolAdminUser, db: Db, within_days: int = Query(30, ge=0, le=365)):
    return [
        {
            "id": i.id,
            "student_id": s.id,
...
def delete_visit(visit_id: int, current_user: SchoolAdminUser, db: Db):
...
def record(student_id: int, current_user: SchoolAdminUser, db: Db):
...
def save_profile(student_id: int, payload: ProfileIn, current_user: SchoolAdminUser, db: Db):
...
def add_checkup(student_id: int, payload: CheckupIn, current_user: SchoolAdminUser, db: Db):
    s = get_school_student(db, student_id, current_user.school_id)
...
def delete_checkup(checkup_id: int, current_user: SchoolAdminUser, db: Db):
...
def add_immunization(student_id: int, payload: ImmunizationIn, current_user: SchoolAdminUser, db: Db):
    s = get_school_student(db, student_id, current_user.school_id)
...
def delete_immunization(imm_id: int, current_user: SchoolAdminUser, db: Db):
