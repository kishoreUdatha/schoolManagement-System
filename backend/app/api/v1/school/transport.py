...
def dashboard(current_user: TransportManager, db: Db):
...
def list_crew(current_user: TransportManager, db: Db):
...
def create_crew(payload: CrewCreate, current_user: TransportManager, db: Db):
...
def update_crew(crew_id: int, payload: CrewUpdate, current_user: TransportManager, db: Db):
...
def delete_crew(crew_id: int, current_user: TransportManager, db: Db):
...
def list_vehicles(current_user: TransportManager, db: Db):
    return [
...
def create_vehicle(payload: VehicleCreate, current_user: TransportManager, db: Db):
...
def get_vehicle(vehicle_id: int, current_user: TransportManager, db: Db):
    v = svc.get_vehicle(db, vehicle_id, current_user.school_id)
    return VehicleRead.model_validate(svc.vehicle_to_read_dict(db, v))


@router.patch("/vehicles/{vehicle_id}", response_model=VehicleRead)
def update_vehicle(vehicle_id: int, payload: VehicleUpdate, current_user: TransportManager, db: Db):
    v = svc.update_vehicle(db, vehicle_id, current_user.school_id, payload)
    return VehicleRead.model_validate(svc.vehicle_to_read_dict(db, v))


@router.delete("/vehicles/{vehicle_id}", status_code=status.HTTP_204_NO_CONTENT)
def delete_vehicle(vehicle_id: int, current_user: TransportManager, db: Db):
...
def rotate_gps_key(vehicle_id: int, current_user: TransportManager, db: Db):
...
def list_logs(vehicle_id: int, current_user: TransportManager, db: Db):
    return [
...
def add_log(vehicle_id: int, payload: VehicleLogCreate, current_user: TransportManager, db: Db):
...
def delete_log(vehicle_id: int, log_id: int, current_user: TransportManager, db: Db):
...
def list_routes(current_user: TransportManager, db: Db):
    return [
...
def create_route(payload: RouteCreate, current_user: TransportManager, db: Db):
...
def get_route(route_id: int, current_user: TransportManager, db: Db):
    r = svc.get_route(db, route_id, current_user.school_id)
    return RouteRead.model_validate(svc.route_to_read_dict(db, r))


@router.patch("/routes/{route_id}", response_model=RouteRead)
def update_route(route_id: int, payload: RouteUpdate, current_user: TransportManager, db: Db):
    r = svc.update_route(db, route_id, current_user.school_id, payload)
    return RouteRead.model_validate(svc.route_to_read_dict(db, r))


@router.delete("/routes/{route_id}", status_code=status.HTTP_204_NO_CONTENT)
def delete_route(route_id: int, current_user: TransportManager, db: Db):
...
def assign(payload: AssignmentCreate, current_user: TransportManager, db: Db):
...
def end_assignment(
    assignment_id: int, payload: AssignmentEnd, current_user: TransportManager, db: Db
):
    return AssignmentRead.model_validate(
...
def create_trip(payload: TripCreate, current_user: TransportManager, db: Db):
...
def get_trip(trip_id: int, current_user: TransportManager, db: Db):
...
def update_trip(trip_id: int, payload: TripUpdate, current_user: TransportManager, db: Db):
    t = svc.update_trip(db, trip_id, current_user.school_id, payload)
    return TripDetail.model_validate(svc.trip_to_read_dict(db, t, with_students=True))


@router.post("/trips/{trip_id}/boarding", response_model=TripDetail)
def mark_boarding(trip_id: int, payload: BoardingBulk, current_user: TransportManager, db: Db):
