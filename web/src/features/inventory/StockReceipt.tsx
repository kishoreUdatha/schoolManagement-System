...
export function StockReceipt() {
  ...
  const thisMonth = (recent.data ?? []).filter((m) => m.kind === "purchase" && m.moved_on >= monthStart());
  const n = (v: number, ready: unknown) => (ready ? v.toLocaleString("en-IN") : "…");
  const stats = [
    { label: "Bought", value: recent.data ? money(thisMonth.reduce((s, m) => s + Number(m.qty) * Number(m.unit_cost ?? 0), 0)) : "…", note: "Received this month, at cost" },
    { label: "Lines received", value: n(thisMonth.length, recent.data), note: "This month" },
    { label: "Low stock", value: n((items.data ?? []).filter((i) => i.low_stock).length, items.data), note: "Below reorder level" },
    { label: "Suppliers", value: n((suppliers.data ?? []).filter((s) => s.is_active).length, suppliers.data), note: "Active" },
  ];
