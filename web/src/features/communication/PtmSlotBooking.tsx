...
function TeacherSlots() {
  ...
  const slotCount = (st: PtmSlot["status"]) => String(m?.slots.filter((x) => x.status === st).length ?? 0);
  const stats = [
    { label: "Booked", value: String(booked), note: `of ${m?.slots.length ?? 0} slots` },
    { label: "Free slots", value: slotCount("open"), note: "Not booked yet" },
    { label: "Met", value: slotCount("done"), note: "Recorded as met" },
    { label: "Missed", value: slotCount("no_show"), note: "Parent did not come" },
  ];
...
function OfficeSlots() {
  ...
  // The chosen meeting's slots, across all its teachers.
  const all = s?.teachers.flatMap((x) => x.slots) ?? [];
  const num = (v: number) => (s ? String(v) : "…");
  const today = new Date().toISOString().slice(0, 10);
  const stats = [
    { label: "Booked", value: num(s?.booked_count ?? 0), note: s ? `of ${s.slot_count} slots` : "This meeting" },
    { label: "Free slots", value: num(all.filter((x) => x.status === "open").length), note: "This meeting" },
    { label: "Met", value: num(all.filter((x) => x.status === "done").length), note: `${all.filter((x) => x.status === "no_show").length} missed` },
    { label: "Upcoming", value: String(sessions.data.filter((x) => x.meeting_date >= today).length), note: "Meetings from today on" },
  ];
