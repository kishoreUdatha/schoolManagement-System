...
export function CommunicationHistory() {
  ...
  const stats = [
    { label: "Messages", value: num(res.data?.count ?? 0), note: "In the chosen dates" },
    { label: "Delivered", value: num(count("delivered") + count("sent")), note: `${count("queued")} still queued` },
    { label: "Read", value: num(res.data ? res.data.rows.filter((r) => r.read_at).length : 0), note: "Opened by the recipient" },
    { label: "Failed", value: num(count("failed")), note: `${count("skipped")} skipped, no contact on file` },
  ];
