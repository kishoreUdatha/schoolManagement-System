...
export function StaffExit() {
  ...
  const daysLeft = c?.last_working_day ? Math.round((new Date(`${c.last_working_day}T00:00:00`).getTime() - new Date(new Date().toDateString()).getTime()) / 86400000) : null;
  const stats = [
    { label: "Areas", value: String(items.length), note: "On the exit checklist" },
    { label: "Cleared", value: String(done), note: `${share}% signed off` },
    { label: "Outstanding", value: String(items.length - done), note: "Areas still to sign off" },
    {
      label: "Last day",
      value: daysLeft === null ? "—" : String(Math.abs(daysLeft)),
      note: daysLeft === null ? "No last working day set" : `${daysLeft >= 0 ? "Days to go" : "Days ago"} · ${date(c?.last_working_day)}`,
    },
  ];
  ...
  return (
    <>
      {c && c.status !== "cancelled" ? <StatStrip items={stats} compact /> : null}
