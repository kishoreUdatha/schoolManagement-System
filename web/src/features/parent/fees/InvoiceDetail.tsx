...
/*
 * PM-024 · Invoice detail: the child's payable fee lines. The parent may pay
 * all of them or pick some (the school's instalments are the separate fee
 * periods); amounts are the school's and are not editable here.
 */
...
export function InvoiceDetail() {
  ...
  return (
    <>
      ...
      {/* Not wired: invoice number and separate late-fee line — the API returns fee lines, not invoices. */}
      <p>{`${payable.length} fee ${payable.length === 1 ? "line" : "lines"} · choose what to pay now`}</p>
      <dl>
        {payable.map((f) => (
          <div key={f.id}>
            <dt>
              <label className="row" style={{ display: "flex", gap: 8, alignItems: "center" }}>
                <input type="checkbox" checked={picked.has(f.id)} onChange={() => toggle(f.id)} aria-label={`Pay ${feeName(f)}`} />
                <span>
                  {feeName(f)}
                  <small style={{ display: "block" }}>{`Due ${date(f.due_date)}${f.is_overdue ? " · overdue" : ""}${Number(f.amount_paid) > 0 ? ` · ${money(f.amount_paid)} paid` : ""}`}</small>
                </span>
              </label>
            </dt>
